...
class WindowManager {
  ...
  _setupWindowEvents(win, winId, accountIndex, url) {
    ...
    win.webContents.on("console-message", async ( e) => {
      ...
      if(e.message && e.message.startsWith("[RPC]")){
        try {
          const rpcHandler = require("../server/rpc-handler");

          const payload = JSON.parse(e.message.replace("[RPC]",""))
          const res = await rpcHandler.handleMethod(payload.method,{
            win_id:win.id,
            ...payload.params
          })
          win.webContents.executeJavaScript(`console.log(${JSON.stringify(res)})`)
        }catch (e) {
          console.error(e)
        }
      }
